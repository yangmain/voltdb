...
public class ClientInterface implements SnapshotDaemon.DaemonInitiator {
    ...
    public void runTimeToLive(String columnName, long ttlValue, int chunkSize, int timeout, TTLManager.TTLTask task) {
            ...
            public void clientCallback(ClientResponse resp) throws Exception {
                ...
                if (!error.isEmpty()) {
                    if (error.indexOf(TTLManager.DR_LIMIT_MSG) > -1) {
                        //In a DR transaction, if DR buffer is over 50MB, the transaction will be aborted.
                        //Thus when a cluster is replicated, a TTL delete will be aborted if the resulting DR buffer
                        //exceeds 50MB. Thus the tuples will not be deleted and TTL can not move forward
                        hostLog.warn("TTL is disabled for table " + task.tableName + ". " + error);
                        task.cancel();
                    }
                } else {
