...
Table* TableCatalogDelegate::constructTableFromCatalog(catalog::Database const& catalogDatabase,
    ...
    bool needsHiddenColumnForMigration = tableTypePersistentWithMigrateStream(static_cast<TableType>(catalogTable.tableType()));
    // index the hidden column
    if (needsHiddenColumnForMigration) {
        vector<int>  migrationHiddenIndexColumnIndices;
        // the last hidden column
        int hiddenIndex = schema->hiddenColumnCount() + schema->columnCount() -1;
        migrationHiddenIndexColumnIndices.push_back(hiddenIndex);
        TableIndexScheme  hiddenIndexScheme = TableIndexScheme("migrationIndex", HASH_TABLE_INDEX,
                migrationHiddenIndexColumnIndices, TableIndex::simplyIndexColumns(),
                true, true, schema);
        TableIndex* migrationIndex = TableIndexFactory::getInstance(hiddenIndexScheme);
        assert(migrationIndex);
        persistentTable->addIndex(migrationIndex);
        VOLT_DEBUG("Creating index on migration hidden column of %s: column index %d", tableName.c_str(), hiddenIndex);
    }
    return table;
}
